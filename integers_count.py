...
def mapper(file_id, tmp_dir, data_dir):
    """
    Map phase: count integers in range 0-100 from assigned data file.
    file_id: SLURM array task ID (0-3)
    """
    # Convert array index to file number (0->1, 1->2, 2->3, 3->4)
    file_number = int(file_id) + 1
    file_path = os.path.join(data_dir, f"project1_data_{file_number}.txt")
